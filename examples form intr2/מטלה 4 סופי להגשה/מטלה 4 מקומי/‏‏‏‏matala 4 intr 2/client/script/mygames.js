...
const apiGet = `https://localhost:7198/api/Games/GetGamesByUserID/${userOBJ[0].id}`;
...
mainD.addEventListener("click", (e) => {
  if (e.target.tagName === "BUTTON") {
    ...
    let apiToDelete = `https://localhost:7198/api/UserGame/RemoveFromUserGameConnection?id=${userOBJ[0].id}&appID=${e.target.id}`;
...
function ajaxBringGamesByPrice() {
  let apiGetByPrice = `https://localhost:7198/api/Games/GetGameByMinPrice/id/${userOBJ[0].id}/minPrice/${inputPrice.value}`;
  if (inputPrice.value == "") {
    ajaxBringGames();
    return true;
  }
...
function ajaxBringGamesByRank() {
  let apiGetByRank = `https://localhost:7198/api/Games/GetGameByMinRank/id/${userOBJ[0].id}/minRank/${inputRank.value}`;
